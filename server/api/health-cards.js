// Standalone serverless function for /api/health-cards
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Extract patientId from URL path (e.g., /api/health-cards/patient/123)
    const urlParts = req.url.split('/');
    const patientIndex = urlParts.indexOf('patient');
    const patientId = patientIndex !== -1 ? urlParts[patientIndex + 1]?.split('?')[0] : null;
    
    console.log('📋 Fetching health card for patient:', patientId || userId);
